...
Shared utility modules used across the entire application. Contains the HTTP client, analytics abstraction, CSS class-name helper, and Excel export function.

## How it works

The central module is `api.ts`, which every hook in `src/hooks/` calls. It reads the JWT from the Supabase session, attaches it as a `Bearer` token, and sends the request to the FastAPI backend. Other modules in this folder provide cross-cutting utilities that multiple components and pages depend on.
...
- **api.ts** -- HTTP client that wraps the Fetch API. The `apiFetch` function gets the JWT from the Supabase session and attaches it as a `Bearer` token on every request. Handles non-OK responses by parsing the JSON error body and throwing an Error. Exports `api.get()`, `api.getList()` (which unwraps the `{ data, total }` pagination envelope), `api.post()`, `api.put()`, and `api.del()`.
