...
- **ItemDetail.tsx** -- Item detail at `/items/:id`, the most complex page. Shows item info with inline editing for description and category. Four sub-tables: supplier quotes (with add/delete), inventory locations with quantities, sales lines (clickable to sale detail), and receipt lines (clickable to receipt detail). Uses 6 separate query hooks.
