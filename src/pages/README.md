...
- **Items.tsx** -- Item list page at `/items`. Renders items in a `DataTable` with computed columns: category name, stock locations (comma-separated), total quantity (sum of balances), and average unit cost (from posted receipt lines). Supports `embedded` mode for the Settings page.
- **ItemDetail.tsx** -- Item detail at `/items/:id`, the most complex page. Shows item info (description, category, total quantity, current supplier price) with inline editing for description and category. The "Current Supplier Price" field displays the cost, currency, and supplier name from the most recent quote. The header includes "New Sale" and "New Receipt" buttons that open `AddSaleModal` / `AddReceiptModal` with the current item pre-filled in the first line. Five sub-sections: supplier quotes (with add/delete), inventory locations with transfer buttons, receipt history (clickable to receipt detail), sales history (clickable to sale detail), and stock movements. The Add Quote modal pre-fills with the latest quote's supplier, cost, and currency. Uses 7 separate query hooks.
